...
namespace blackbone
{
    ProcessModules::ProcessModules(class Process &proc)
        : _proc(proc), _memory(_proc.memory()), _core(_proc.core()), _ldrPatched(false)
    {
    }
        ...
        if (expBase)
        {
            ...
            if (expSize <= sizeof(IMAGE_EXPORT_DIRECTORY))
            {
                ...
                expSize = static_cast<DWORD>(
                    pExpData->AddressOfNameOrdinals - expBase + max(pExpData->NumberOfFunctions, pExpData->NumberOfNames) * 255);
